...
const FEATURES = [
  {
    icon: "⚡",
    heading: "Live in 2 Minutes",
    body: "Fill the form, verify OTP — your listing goes live instantly.",
  },
  {
    icon: "🔒",
    heading: "Hide Your Number",
    body: "Stay private until you're ready. Only OTP-verified buyers reach you.",
  },
  {
    icon: "📊",
    heading: "Real-time Dashboard",
    body: "Track views, saves & enquiries from one smart dashboard.",
  },
  {
    icon: "🏆",
    heading: "RERA Verified Badge",
    body: "Get a trust badge that makes buyers 3× more likely to contact you.",
  },
];
...
const TESTIMONIALS = [
  {
    ...
    stars: 5,
    quote:
      "Listed my flat in 2 minutes and got 12 enquiries the same day. Unbelievably fast!",
  },
  {
    name: "Priya Mehta",
    role: "Agent",
    ...
    stars: 5,
    quote:
      "Best portal for independent agents. Verification is smooth and leads are genuine.",
  },
  {
    name: "Rohit Verma",
    role: "Builder",
    ...
    stars: 5,
    quote:
      "Sold 3 units from a single paid listing. The ROI is incredible compared to other portals.",
  },
  {
    name: "Sunita Kapoor",
    role: "Property Owner",
    ...
    stars: 5,
    quote:
      "Got a tenant in 4 days! Would never go back to newspaper ads or other apps.",
  },
];
  ...
  Spinner: ({ dark = false }: { dark?: boolean }) => (
    <svg className="animate-spin w-4 h-4" fill="none" viewBox="0 0 24 24">
      <circle
        ...
        cy="12"
        r="10"
        stroke={dark ? "#1E40AF" : "white"}
        strokeWidth="4"
      />
      <path
        className="opacity-75"
        fill={dark ? "#1E40AF" : "white"}
...
function AnimCounter({ target, suffix }: { target: number; suffix: string }) {
  ...
  return (
    <em ref={ref} style={{ fontStyle: "normal" }}>
      {val}
      {suffix}
    </em>
  );
...
function OtpInput({
  ...
  const boxes = Array.from({ length: 6 }, () => useRef<HTMLInputElement>(null));
  ...
  const handleInput = (i: number, raw: string) => {
    ...
    if (d && i < 5) {
      setTimeout(() => boxes[i + 1].current?.focus(), 10);
    }
  };
  ...
  const handlePaste = (e: React.ClipboardEvent) => {
    ...
    onChange(p);
    boxes[Math.min(p.length, 5)].current?.focus();
  };
      ...
      <div className="grid grid-cols-6 gap-2 sm:gap-3">
        {digits.map((d, i) => (
          <input
            ...
            className={`w-full aspect-square text-center font-black border-2 rounded-xl outline-none transition-all duration-200
              ${
                d.trim()
                  ? "border-[#2563EB] bg-blue-50 text-[#1E40AF] shadow-md"
...
export default function Think4BuySaleLanding() {
  ...
  useEffect(() => {
    const t = setInterval(
      () => setTestIdx((p) => (p + 1) % TESTIMONIALS.length),
      4000,
    );
  ...
  const validate = () => {
    ...
    if (!form.name || form.name.trim().length < 3)
      e.name = "Enter your full name (min 3 chars)";
  ...
  const sendOtp = async () => {
    ...
    try {
      const raw = await sendOtpApi({ phone: form.phone });

      // Handle axios style or direct response
      ...
      if (res?.otp) {
        const otpValue = res.otp.toString().slice(0, 6);
        setForm((p) => ({ ...p, otp: otpValue }));
      }
    } catch (err: any) {
      setApiError(
  ...
  useEffect(() => {
    if (step === "otp" && form.otp.length === 6 && !loading) {
      verifyOtp();
    }
  }, [form.otp]);
  ...
  return (
    <>
      ...
      <div className="min-h-screen mesh relative overflow-x-hidden">
        ...
        <div
          aria-hidden
          className="pointer-events-none absolute top-0 left-1/4 w-[500px] h-[500px] rounded-full blur-[140px]"
          style={{
            background:
        ...
        <div
          aria-hidden
          className="pointer-events-none absolute bottom-1/3 right-0 w-[400px] h-[400px] rounded-full blur-[120px]"
          style={{
            background:
        ...
        <header role="banner">
          ...
          >
            <a
              href="/"
              aria-label="Think4BuySale Home"
              className="flex items-center gap-2.5 no-underline"
            >
              <div
                className="w-9 h-9 rounded-xl flex items-center justify-center"
                style={{
                  background: "linear-gradient(135deg,#0B3C8C,#1E40AF,#2563EB)",
            ...
            <div className="hidden md:flex items-center gap-6" role="menubar">
              {["Browse Properties", "New Projects", "Agencies", "Blog"].map(
                (l) => (
                  <a
                    key={l}
                    href="#"
        ...
        <main>
              ...
              <div className="order-1 lg:order-2 card-in">
                ...
                >
                  ...
                  <div
                    ...
                    style={{
                      background:
                        "linear-gradient(90deg,#0B3C8C 0%,#1E40AF 35%,#2563EB 70%,#3B82F6 100%)",
                    }}
                  />
                  ...
                  {step !== "success" && (
                    <div
                      className="flex items-center gap-1 px-6 pt-5 mb-1"
                      aria-label="Form progress"
                      role="progressbar"
                    >
                      {["Your Details", "Verify OTP", "You're In!"].map(
                        (l, i) => {
                          const cur =
                            step === "form" ? 0 : step === "otp" ? 1 : 2;
                          const done = i < cur,
                            active = i === cur;
                          return (
                            <div
                            ...
                            >
                              <div
                                className={`w-6 h-6 rounded-full flex items-center justify-center border-2 text-[9px] font-black flex-shrink-0 transition-all duration-400
                              ...
                              <span
                                className={`hidden sm:block text-[10px] font-bold transition-colors ${active ? "text-[#1E40AF]" : done ? "text-emerald-500" : "text-slate-300"}`}
                              >
                                {l}
                              </span>
                              {i < 2 && (
                                <div
                                  className={`flex-1 h-px rounded-full min-w-[8px] transition-all duration-500 ${done ? "bg-emerald-400" : "bg-slate-200"}`}
                                />
                              )}
                            </div>
                          );
                  ...
                  <div className="px-6 pb-6 pt-4 md:px-8 md:pb-8">
                    ...
                    {apiError && (
                      <div
                        className="fade-dn mb-4 flex items-start gap-2.5 bg-rose-50 border border-rose-200 rounded-2xl px-4 py-3"
                        role="alert"
                      >
                        <span className="text-rose-500 text-sm flex-shrink-0">
                          ⚠
                        </span>
                        <p className="text-rose-600 text-[13px] font-semibold leading-snug flex-1">
                          {apiError}
                        </p>
                        <button
                          onClick={() => setApiError("")}
                    ...
                    {step === "form" && (
                      <div className="slide-in">
                        ...
                        <div className="mb-4">
                          <p
                            className="text-[10px] font-black uppercase tracking-[0.15em] text-slate-500 mb-2"
                            id="intent-label"
                          >
                            I want to
                          </p>
                          <div
                            ...
                            className="flex p-1 bg-slate-100 rounded-2xl gap-1"
                          >
                            {[
                              { v: "sell", l: "📈 Sell" },
                              { v: "rent", l: "🔑 Rent / Lease" },
                          ...
                          >
                            {PROPERTY_TYPES.map((t) => (
                              ...
                              >
                                ...
                                <div>
                                  <p
                                    className={`text-xs font-black leading-none ${form.propertyType === t.v ? "text-[#1D4ED8]" : "text-slate-700"}`}
                                  >
                                    {t.l}
                                  </p>
                        ...
                        <div className="mb-3">
                          <label
                            htmlFor="city"
                            className="text-[10px] font-black uppercase tracking-[0.15em] text-slate-500 block mb-2"
                          >
                            City{" "}
                            <span
                        ...
                        <button
                          ...
                          style={{
                            background:
                              "linear-gradient(135deg,#0B3C8C 0%,#1E40AF 50%,#2563EB 100%)",
                            boxShadow:
                              "0 8px 32px rgba(11,60,140,0.45), 0 2px 0 rgba(255,255,255,0.05) inset",
                            WebkitTapHighlightColor: "transparent",
                          }}
                        >
                          {loading ? (
                    ...
                    {step === "otp" && (
                      <div className="slide-in">
                        ...
                        <h2 className="display text-[22px] font-bold text-[#0B3C8C] mb-1">
                          Verify Your Number
                        </h2>
                        <p className="text-slate-500 text-sm mb-1">
                          OTP sent to{" "}
                          <strong className="text-[#1E40AF] font-black">
                            +91 {form.phone}
                          </strong>
                        </p>
                        <button
                        ...
                        <OtpInput
                          value={form.otp}
                          onChange={(v) => upd("otp", v)}
                        />
                        {errors.otp && (
                          <p
                            role="alert"
                            className="text-rose-500 text-[11px] font-semibold text-center -mt-2 mb-3 flex items-center justify-center gap-1"
                          >
                            ⚠ {errors.otp}
                          </p>
                        )}
                      ...
                      <div className="slide-in text-center py-3">
                        <div
                          ...
                          style={{
                            ...
                            animation:
                              "popBounce 0.7s cubic-bezier(0.34,1.56,0.64,1) both",
                          }}
                          aria-hidden
                        >
                          <svg
                        ...
                        <h2 className="display text-2xl font-bold text-[#0B3C8C] mb-2">
                          You're Verified! 🎉
                        </h2>
                        <p className="text-slate-500 text-sm leading-relaxed mb-5 max-w-xs mx-auto">
                          Your details are saved. Complete your full listing to
                          get{" "}
                          <strong className="text-[#1E40AF]">
                            5× more buyer enquiries.
                          </strong>
                        </p>
                        <div className="grid grid-cols-2 gap-2 mb-5 text-left">
                          {[
                            ...
                            <div
                              key={l}
                              className="flex items-center gap-2.5 bg-blue-50/60 border border-blue-100 rounded-2xl p-3"
                            >
                        ...
                        <button
                          ...
                          style={{
                            background:
                              "linear-gradient(135deg,#0B3C8C,#1E40AF,#2563EB)",
                            boxShadow: "0 8px 28px rgba(11,60,140,0.4)",
                            WebkitTapHighlightColor: "transparent",
                          }}
                        ...
                        >
                          <span className="flex items-center justify-center gap-2">
                            Complete Your Listing <IC.Arrow />
                          </span>
                        </button>
                        <button
                          onClick={() => {
              ...
              <div className="order-2 lg:order-1 text-white hero-in">
                <div className="inline-flex items-center gap-2 mb-6 px-4 py-2 rounded-full border border-white/15 glass">
                  ...
                  <span className="text-xs font-bold text-white/90">
                    India's Fastest Growing Realty Platform
                  </span>
                </div>

                <h2 className="display font-bold leading-[1.1] mb-5">
                  <span className="block text-[32px] md:text-[42px] lg:text-[50px]">
                    Sell or Rent
                  </span>
                  <span className="block text-[32px] md:text-[42px] lg:text-[50px]">
                    Your Property
                  ...
                  <span
                    ...
                    style={{
                      background:
                        "linear-gradient(90deg,#93c5fd,#3B82F6,#2563EB)",
                ...
                <p className="text-white/65 text-[15px] leading-relaxed mb-6 max-w-md">
                  ...
                  across India who trust Think4BuySale to connect them with
                  verified, genuine buyers and tenants.
                </p>

                <ul
                  className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 mb-8 list-none p-0 m-0"
                ...
                <div className="grid grid-cols-4 gap-3 mb-8 pb-8 border-b border-white/10">
                  {STATS.map((s) => (
                    <div key={s.label} className="text-center">
                      ...
                      <p className="text-white/45 text-[9px] font-bold uppercase tracking-wider leading-tight">
                        {s.label}
                      </p>
                    </div>
                  ))}
                ...
                <div className="grid grid-cols-2 gap-2.5 mb-8">
                  {FEATURES.map((f, fi) => (
                    <div
                      key={f.heading}
                      className="glass rounded-2xl p-3.5 hover:bg-white/8 transition-colors"
                    >
                      <p
                        className="text-2xl mb-2 float"
                        style={{ animationDelay: `${fi * 0.4}s` }}
                      >
                        {f.icon}
                      </p>
                      ...
                      <p className="text-white/45 text-[10px] leading-relaxed hidden sm:block">
                        {f.body}
                      </p>
                    </div>
                  ))}
                ...
                {/* Testimonials */}
                <div>
                  <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/35 mb-3">
                    Trusted by Property Owners
                  </p>
                  <div className="glass rounded-2xl p-4 mb-3" key={testIdx}>
                    <div className="flex gap-0.5 mb-2.5">
                      {Array(TESTIMONIALS[testIdx].stars)
                        .fill(0)
                        .map((_, i) => (
                          <IC.Star key={i} />
                        ))}
          ...
          {/* ── MARQUEE ─────────────────────────────────────────────────── */}
          <div
            className="relative z-10 border-t border-white/8 bg-white/3 overflow-hidden py-4"
            ...
            <div className="flex marquee-track gap-12 whitespace-nowrap">
              {[...Array(2)].flatMap((_, gi) =>
                [
                  ...
                  <span
                    key={`${gi}-${ii}`}
                    className="inline-flex items-center gap-2.5 text-white/40 text-[11px] font-bold"
                  >
          ...
          >
            <div className="text-center mb-10">
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#3B82F6] mb-2">
                Simple Process
              </p>
              <h2
                id="how-heading"
                className="display text-3xl font-bold text-white mb-3"
              >
                How it Works
              </h2>
              <p className="text-white/50 text-sm max-w-sm mx-auto">
                Post your property in 3 easy steps and start receiving genuine
                enquiries today
              </p>
            </div>
              ...
              {[
                {
                  ...
                  body: "Select property type, enter your city and contact number. Takes under 60 seconds.",
                },
                {
                  step: "02",
                  icon: "🔐",
                  title: "Verify via OTP",
                  body: "Enter the 4-digit OTP sent to your mobile. Your number is instantly verified.",
                },
                {
                  step: "03",
                  icon: "🚀",
                  title: "Go Live & Get Leads",
                  body: "Complete your full listing with photos and price to start receiving genuine buyer enquiries.",
                },
              ].map((s) => (
                <div
                  key={s.step}
                  className="glass rounded-3xl p-6 text-center hover:bg-white/6 transition-all group"
                >
                  <div className="text-5xl mb-4 group-hover:scale-110 transition-transform duration-300">
                    {s.icon}
                  </div>
                  <div
                    className="w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-black text-white mx-auto mb-3"
                    style={{
                      background: "linear-gradient(135deg,#1E40AF,#2563EB)",
                    }}
                  >
                  ...
                  <h3 className="display text-base font-bold text-white mb-2">
                    {s.title}
                  </h3>
        ...
        <footer
          role="contentinfo"
          className="relative z-10 border-t border-white/8 max-w-[1280px] mx-auto px-4 md:px-8 py-8"
        >
