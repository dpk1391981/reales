      ...
      {steps.map((s, i) => (
        <div key={s} className="flex items-center gap-2">
          <div className={`rounded-full transition-all duration-300 flex items-center justify-center
            ${i < idx ? "w-6 h-6 bg-emerald-500" :
              i === idx ? "w-6 h-6 bg-[#0f2342]" :
              "w-2 h-2 bg-slate-200"}`}>
            {i < idx && <CheckIcon />}
            {i === idx && <span className="text-white text-[9px] font-bold">{i + 1}</span>}
          </div>
          {i < steps.length - 1 && (
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-[999] flex items-end sm:items-center justify-center
        ...
        <div className="modal-enter relative bg-white
          ...
          {step === "details" && (
            <div className="px-5 sm:px-6 pb-6">
              ...
              {/* Google */}
              <button className="w-full flex items-center justify-center gap-3 border-2 border-slate-200
              ...
              <Field label="Mobile Number" icon={<PhoneIcon />}>
                <div className={`flex border rounded-2xl overflow-hidden bg-slate-50
                  ...
                  <span className="flex items-center px-3 text-sm font-bold text-[#0f2342]
              ...
              <p className="text-center text-xs text-slate-400 mt-4">
                ...
                <button className="text-amber-500 font-semibold border-none bg-transparent
          ...
          {step === "otp" && (
            <div className="px-5 sm:px-6 pb-6">
              ...
              >
                {loading ? (
                  <svg className="animate-spin w-4 h-4" fill="none" viewBox="0 0 24 24">
                    <circle className="opacity-25" cx="12" cy="12" r="10" stroke="#0f2342" strokeWidth="4" />
                    <path className="opacity-75" fill="#0f2342" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
                  </svg>
                ) : null}
          ...
          {step === "success" && (
            <div className="px-5 sm:px-6 pb-8 flex flex-col items-center text-center">
              <div className="success-pop w-20 h-20 rounded-full bg-gradient-to-br
              ...
              <p className="text-sm text-slate-500 max-w-[260px] leading-relaxed mb-6">
                Welcome to Think4BuySale. Explore thousands of verified properties across India.
              </p>

              {/* Benefits */}
              <div className="w-full bg-slate-50 rounded-2xl p-4 mb-6 text-left">
                {["Save properties to wishlist", "Get price alerts", "Chat with agents directly", "Exclusive project launches"].map((b) => (
                  <div key={b} className="flex items-center gap-2.5 py-2
                    border-b border-slate-100 last:border-0">
                    <span className="w-5 h-5 rounded-full bg-emerald-100 flex items-center
